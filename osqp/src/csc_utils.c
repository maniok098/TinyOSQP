...
OSQPCscMatrix* csc_spalloc(OSQPInt m,
                           ...
                           OSQPInt values,
                           OSQPInt triplet) {
  OSQPCscMatrix* A = (OSQPCscMatrix*)c_calloc(1, sizeof(OSQPCscMatrix)); /* allocate the OSQPCscMatrix struct */
  ...
  A->p     = (OSQPInt*)csc_malloc(triplet ? nzmax : n + 1, sizeof(OSQPInt));
  A->i     = values ? (OSQPInt*)csc_malloc(nzmax,  sizeof(OSQPInt)) : OSQP_NULL;
  A->x     = values ? (OSQPFloat*)csc_malloc(nzmax,  sizeof(OSQPFloat)) : OSQP_NULL;
...
OSQPCscMatrix* triplet_to_csc(const OSQPCscMatrix* T, OSQPInt* TtoC) {
    ...
    w = (OSQPInt*)csc_calloc(n, sizeof(OSQPInt));                  /* get workspace */
...
OSQPCscMatrix* triplet_to_csr(const OSQPCscMatrix* T, OSQPInt* TtoC) {
  ...
  nz = T->nz;
  C  = csc_spalloc(m, n, nz, Tx != OSQP_NULL, 0);     /* allocate result */
  w  = (OSQPInt*)csc_calloc(m, sizeof(OSQPInt));                  /* get workspace */
...
OSQPInt* csc_pinv(const OSQPInt* p, OSQPInt n) {
  ...
  pinv = (OSQPInt*)csc_malloc(n, sizeof(OSQPInt));     /* allocate result */
