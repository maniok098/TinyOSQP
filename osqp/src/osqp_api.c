...
OSQPInt osqp_setup(OSQPSolver**         solverp,
  ...
  solver = (OSQPSolver*)c_calloc(1, sizeof(OSQPSolver));
  ...
  *solverp = solver;
  ...
  solver->info = (OSQPInfo*)c_calloc(1, sizeof(OSQPInfo));
...
# endif /* ifdef OSQP_ENABLE_PROFILING */
  ...
  work->data = (OSQPData*)c_calloc(1, sizeof(OSQPData));
  ...
  // Allocate solution
  solver->solution = (OSQPSolution*)c_calloc(1, sizeof(OSQPSolution));
  if (!(solver->solution)) return osqp_error(OSQP_MEM_ALLOC_ERROR);
  solver->solution->x             = (OSQPFloat*)c_calloc(1, n * sizeof(OSQPFloat));
  solver->solution->y             = (OSQPFloat*)c_calloc(1, m * sizeof(OSQPFloat));
  solver->solution->prim_inf_cert = (OSQPFloat*)c_calloc(1, m * sizeof(OSQPFloat));
  solver->solution->dual_inf_cert = (OSQPFloat*)c_calloc(1, n * sizeof(OSQPFloat));
...
# ifdef OSQP_ENABLE_DERIVATIVES
  work->derivative_data = (OSQPDerivativeData*)c_calloc(1, sizeof(OSQPDerivativeData));
...
OSQPInt osqp_solve(OSQPSolver *solver) {
  ...
  compute_obj = 0; // No need to compute objective function for embedded mode
  ...
  for (iter = 1; iter <= max_iter; iter++) {
    ...
    if (can_check_termination) {
      ...
      if (check_termination(solver, 0)) { break;}
    }

    // Should we update rho ?
    #ifndef OSQP_DISABLE_RHO_ADAPTION
      if (solver->settings->adaptive_rho && !solver->settings->adaptive_rho_interval) {
    ...
    #endif // !OSQP_DISABLE_RHO_ADAPTION
  }        // End of ADMM for loop of solving
...
OSQPInt osqp_update_data_vec(OSQPSolver*      solver,
  ...
  if (l_new || u_new) {
    ...
    if (solver->settings->scaling) {
      ...
      #endif /* #ifndef OSQP_DISABLE_RHO_ADAPTION  */
