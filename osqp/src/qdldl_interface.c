...
static OSQPInt permute_KKT(OSQPCscMatrix** KKT,
    ...
    if (!PtoKKT && !AtoKKT && !rhotoKKT){  // No vectors to be stored
        ...
        KtoPKPt = (OSQPInt*)c_malloc((*KKT)->p[(*KKT)->n] * sizeof(OSQPInt));
...
OSQPInt init_linsys_solver_qdldl(qdldl_solver**      sp,
    ...
    qdldl_solver* s = (qdldl_solver*)c_calloc(1, sizeof(qdldl_solver));
    *sp = s;
    ...
    s->L = (OSQPCscMatrix*)c_calloc(1, sizeof(OSQPCscMatrix));
    s->L->m  = n_plus_m;
    s->L->n  = n_plus_m;
    s->L->nz = -1;
